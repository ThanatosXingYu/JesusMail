...
- **邮箱收发统计**：管理员邮箱列表显示每个邮箱在现存邮件日志中的成功收、发记录数
...
> 说明：部分容器服务名、路径、数据库名、模块名和环境变量仍保留内部兼容标识，这是为了保证已有部署可安全升级。它们属于实现细节，不代表产品品牌。
...
后台「概览」顶部新增「邮箱整体概况」，统计当前系统中所有现存邮箱（可按域名筛选）的总数、启用数及成功收发日志记录数。它不受上方日期选择影响；下方送达率、打开率、点击率和图表仍只分析所选时段的发信数据。汇总接口错误时显示错误提示和重试按钮，而不是以 0 冒充正常结果。

后台「邮箱」列表的「已收邮件」「已发邮件」按当前页邮箱地址批量查询邮件日志：分别统计收件日志中 `recipient` 与发件日志中 `sender` 为该邮箱、且 `status = sent`、`dsn` 以 `2.` 开头的成功记录。无匹配记录时显示 0。该值是**现存成功日志记录数**，不是邮箱中现存邮件数，也不是永久累计数；日志清理会使数字下降，按 Postfix 队列 ID 保留最新记录的机制可能无法逐个统计群发邮件的所有收件人。统计查询出错时邮箱列表仍可访问，但本次显示为 0，应结合服务日志排查；不可将 0 当成完整审计数据。

登录信息列只提供复制；一键登录在操作列，仍受邮箱有效状态限制。
...
```shell
# 1. 先做好完整备份
# 2. 从现有部署配置中设置 DB_USER 和 DB_NAME
# 3. 执行迁移
psql -U "$DB_USER" -d "$DB_NAME" -f docs/migrations/20260923-permanent-activation-mailboxes.sql
```
