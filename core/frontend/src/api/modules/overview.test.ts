...
describe('overview API', () => {
	...
	it('getMailboxOverviewStats requests all-date totals with only a domain filter', () => {
		getMailboxOverviewStats('example.com')
		expect(mockGet).toHaveBeenCalledWith('/overview/mailbox_stats', {
	...
	it('getSendQueueInfo calls GET /postfix_queue/queue_info', () => {
		...
		expect(mockGet).toHaveBeenCalledWith('/postfix_queue/queue_info', {
			params: { queue_id: 'abc123' },
		})
	})
